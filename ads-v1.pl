...
our $Loaded;
my $Coucil_ID='DEHPR19';

my $input_url = 'https://www.adsgroup.org.uk/community/cms/content/preview/nlrss.asp?p=9574';
my $List_content = &Get_Content($input_url);

while($List_content =~ m/<item>\s*([\w\W]*?)\s*<\/item>/igs)
{
	my $single_news_content = $1;
	my ($title,$news_page_link);
	if($single_news_content =~ m/<title>\s*([\w\W]*?)\s*<\/title>/is)
	{
		$title = &Trim($1);
	}
	if($single_news_content =~ m/<link>\s*([\w\W]*?)\s*<\/link>/is)
	{
		$news_page_link = $1;
	}
