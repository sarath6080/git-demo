...
our $Loaded;
my $Coucil_ID='DEHPR11';

my $input_url = 'http://www.acpo.presscentre.com/Content/Default.aspx?NewsAreaID=2';
...
while($List_content =~ m/<h3><a[^>]*?href\=\"([^>]*?)\"/igs)
{
	my $title = &Trim($1);
	my $news_page_link = $1;
...
sub collect_information()
{
	...
	$typecode = '106010';
	$onhold = 'True';
	if($content =~ m/<h2\s*class\=\"page_ttl\">\s*([^>]*?)\s*<\/div>/is)
	{
		$header = &Trim($1);
		$header = WebContent_Trim($header);
		# $header = &DB_Dehaviland::WebContent_Trim($header);
	}
	if($content =~ m/greyDate\">\s*([^>]*?)\s*</is)
	{
		$event_date = &Trim($1);
		if($event_date =~ m/(\d+)\W([a-z]+)\W(\d+)/is)
		{
			$event_date = $3.'-'.$MonthHash{uc($2)}.'-'.$1.' 00:00:00.0';
		}
		
	}
	if($content =~ m/(<div\s*class\=\"bodyCopy\">[\w\W]*?<div\s*class\=\"clearBlock\">)/is)
	{
